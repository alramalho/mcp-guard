...
async function getRunningPid(port: number): Promise<number | undefined> {
...
function turnOff(pid: number): void {
  process.kill(pid, "SIGTERM");
  unlinkSync(PID_FILE);
  console.log(red("MCP Guard off"));
}

function turnOn(configPath: string, port: number): void {
...
function findConfigUp(): string | undefined {
  let dir = resolve(".");
  const root = homedir();
  while (true) {
...
function loadConfig(configPath: string): GuardConfig {
  ...
  if (!config.port) config.port = 6427;
...
function parseArgs(argv: string[]): { configPath?: string; serve: boolean; debug: boolean } {
